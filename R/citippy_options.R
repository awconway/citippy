...
#' @examples
#' citippy_options()
#' citippy_options("first.inits", "bib.style")
#' 
#' oldopts <- citippy_options(first.inits = FALSE, bib.style = "authoryear")
#' oldopts
...
citippy_options <- function(..., ref_path, restore.defaults = FALSE){
  ...
  if (missing(...))
    return(mget(.BibOptNames, envir = .citippy_options))
  opts <- list(...)
  nom <- names(opts)
  
  if (is.null(nom)  && !is.list(opts[[1L]])){
    opts <- unlist(opts)
    return(mget(opts[opts %in% .BibOptNames], envir = .citippy_options))
  }else{
    if (is.list(opts[[1L]])){
      opts <- opts[[1L]]
      nom <- names(opts)
    ...
    if (any(!nom %in% .BibOptNames))
      stop('Invalid name specified, see ?citippy_options')
    ...
    if (any(ind)){
      ...
      if (any(is.na(opts[ind])))
        stop(gettextf("One of the specified option values should be logical %s",
                      "and is not, see ?citippy_options"))
      names(opts[ind]) <- nom[ind]
    }
    
    oldopts <- mget(nom, envir=.citippy_options)
